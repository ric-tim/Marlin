...
void check_axes_activity()
{
  ...
  #ifdef REVOL
  if((DISABLE_R) && (r_active == 0)) disable_r(); 
  #endif
...
block->steps_y = labs((target[X_AXIS]-position[X_AXIS]) - (target[Y_AXIS]-position[Y_AXIS]));
  ...
  block->step_event_count = max(block->steps_x, max(block->steps_y, max(block->steps_z, max(block->steps_e, block->steps_r))));
   #else
...
#ifdef REVOL
// Add a new arc movement to the buffer. steps_x, _y and _z is the absolute position in 
// mm. Microseconds specify how many microseconds the move should take to perform. To aid acceleration
// calculation the caller must also provide the physical length of the line in millimeters.

void plan_buffer_arc(const float &x, const float &y, const float &z, const float &e, const float &angular_travel, const float
millimeters_of_travel, uint8_t isclockwise, float feed_rate, const uint8_t &extruder)
{
  // Calculate the buffer head after we push this byte
  int next_buffer_head = next_block_index(block_buffer_head);
...
#ifndef COREXY
// default non-h-bot planning
// revol X et Y ne bougent pas mais il faut mettre à jour position X_AXIS,position Y_AXIS et position Z_AXIS
block->steps_x = labs(target[X_AXIS]-position[X_AXIS]);
block->steps_y = labs(target[Y_AXIS]-position[Y_AXIS]);
#else
// corexy planning
...
block->steps_y = labs((target[X_AXIS]-position[X_AXIS]) - (target[Y_AXIS]-position[Y_AXIS]));
  ...
  {
    block->millimeters = fabs(delta_mm[R_AXIS]);
  }
  float inverse_millimeters = 1.0/block->millimeters;  // Inverse millimeters to remove multiple divides 
...
#define MAX_FREQ_TIME (1000000.0/XY_FREQUENCY_LIMIT)
  ...
  else
  {
	...
	if(((float)block->acceleration_st * (float)block->steps_r / (float)block->step_event_count) > axis_steps_per_sqr_second[R_AXIS])
    	// acceleration is too fast for y, so limit acceleration to a rate that just passes the test on previous line
    	block->acceleration_st = axis_steps_per_sqr_second[R_AXIS] * (float)block->step_event_count/(float)block->steps_r;
...
#ifdef REVOL
void plan_set_r_position(const float &r)
{
  position[R_AXIS] = lround(r*axis_steps_per_unit[R_AXIS]);  
  st_set_r_position(position[R_AXIS]);
}
#endif
